...
static inline int LoadFile(BspFile* bspFile, const char* path) 
{
    if (!bspFile) return -1;
...
// Close the file
static inline void UnloadFile(BspFile* bspFile) 
{
...
static inline void Seek(BspFile* bspFile, long offset) 
{
    if (bspFile && bspFile->file) {
...
int bsp_load_miptextures(BspFile* bsp) 
{
    ...
    if (offset == -1) {
        ...
        memset(&bsp->mip_textures[i], 0, sizeof(miptex_t)); // optional
        continue;
    }
...
static int bsp_load_lightmaps(BspFile *bsp)
{
    ...
    Seek(bsp, lightmaps_lump->offset);
